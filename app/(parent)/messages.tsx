...
const SOFT   = theme.color.parentSoft;
...
function Avatar({ name, photo, size = 44, color = ACCENT }: { name: string; photo?: string | null; size?: number; color?: string }) {
...
function ConvItem({ item, onPress, active }: { item: Conversation; onPress: () => void; active: boolean }) {
  return (
    <TouchableOpacity style={[s.convItem, active && s.convItemActive]} onPress={onPress} activeOpacity={0.75}>
      <View style={s.convAva}>
        <Avatar name={item.partner_name} photo={item.partner_photo} size={46} color={theme.color.helper} />
        {item.unread_count > 0 && (
...
function ChatPanel({ partnerId, partnerName, partnerPhoto, jobPostId, onBack }: {
  ...
  if (loading) return <View style={s.chatLoadWrap}><ActivityIndicator color={ACCENT} /></View>;
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={s.chatHeader}>
        ...
        <Avatar name={partnerName} photo={partnerPhoto} size={36} color={theme.color.helper} />
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: CANVAS }}>
          ...
          {loadingConvs ? (
            ...
            <View style={s.emptyWrap}>
              ...
              <Text style={s.emptySub}>When helpers apply to your jobs, you can message them here.</Text>
            </View>
          ) : (
            <FlatList
              data={conversations}
    ...
    return (
      <>
        ...
        <ConfirmationModal
          ...
          onConfirm={async () => { setConfirmLogoutVisible(false); setTimeout(() => handleLogout(), 1500); }}
...
const s = StyleSheet.create({
  desktopRoot:  { flex: 1, flexDirection: 'row', backgroundColor: CANVAS },
  ...
  convItemActive: { backgroundColor: SOFT },
  ...
  convJob:        { fontSize: 11, color: ACCENT, marginTop: 1 },
