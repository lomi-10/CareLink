...
try {
    ...
    $household = null;
    if ($profile_id !== null) {
        $householdSql = "SELECT household_id, household_size, household_type, has_children, has_elderly, has_pets, pet_details, created_at, updated_at FROM parent_household WHERE profile_id = ?";
    ...
    if ($profile !== null) {
        ...
        $total += 5;
        if ($household !== null && !empty(trim((string) (isset($household['household_type']) ? $household['household_type'] : '')))) {
            $completed += 5;
        }
