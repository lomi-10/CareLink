...
try {
    ...
    try {
        ...
        if ($householdResult->num_rows > 0) {
            $householdUpdateStmt = $conn->prepare("UPDATE parent_household SET household_size = ?, household_type = ?, has_children = ?, has_elderly = ?, has_pets = ?, pet_details = ?, updated_at = NOW() WHERE profile_id = ?");
            $householdUpdateStmt->bind_param("isiiisi", $household_size, $household_type, $has_children, $has_elderly, $has_pets, $pet_details, $profile_id);
            $householdUpdateStmt->execute();
            $householdUpdateStmt->close();
        } else {
            $householdInsertStmt = $conn->prepare("INSERT INTO parent_household (profile_id, household_size, household_type, has_children, has_elderly, has_pets, pet_details, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
            $householdInsertStmt->bind_param("iisiiis", $profile_id, $household_size, $household_type, $has_children, $has_elderly, $has_pets, $pet_details);
        ...
        // Notify the parent that their profile was saved
        createNotification(
            $conn,
            $user_id,
            'profile_update',
