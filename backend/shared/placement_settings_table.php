...
// attendance-tracking opt-out: attendance is a shared record-keeping aid, not
// surveillance, so a parent can turn it OFF for a placement. Default is ON.
...
if (!function_exists('ensure_placement_settings_table')) {
    ...
    /** True if attendance tracking is enabled for a placement (default ON). */
    function get_attendance_tracking(mysqli $conn, int $application_id): bool
    {
        ...
        $stmt->close();
        return $row ? ((int) $row['attendance_tracking'] === 1) : true;
    }
