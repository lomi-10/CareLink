...
function Chip({
  ...
  return (
    ...
    >
      <Text style={[styles.chipLabel, { color: chipText }]} numberOfLines={2}>
        {label}
      </Text>
    </Pressable>
  );
...
export function SpecialtiesShowcase({ jobs, skills, languages, onPressEdit }: Props) {
  const blocks = useMemo(() => BLOCKS(jobs, skills, languages), [jobs, skills, languages]);
    ...
    <View style={styles.card}>
      ...
      <View style={styles.headRow}>
        <View style={styles.headLeft}>
          <View style={[styles.iconWrap, { backgroundColor: theme.color.helperSoft }]}>
            <Ionicons name="sparkles" size={22} color={theme.color.helper} />
          </View>
          <View>
        ...
        >
          ...
          {block.items.filter(Boolean).length === 0 ? (
            <View style={styles.emptyRow}>
              <Ionicons name="add-circle-outline" size={18} color={theme.color.muted} />
