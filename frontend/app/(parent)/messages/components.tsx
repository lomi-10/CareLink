    ...
    return (
      <View style={[s.bubbleWrap, isMine ? s.bubbleWrapRight : s.bubbleWrapLeft]}>
        <View style={{ maxWidth: 330, backgroundColor: '#fff', borderRadius: 16, borderWidth: 1, borderColor: '#EDE0D0', overflow: 'hidden' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9, paddingHorizontal: 12, paddingVertical: 10, backgroundColor: CARAMEL + '22' }}>
            <View style={{ width: 34, height: 34, borderRadius: 10, backgroundColor: ACCENT + '22', alignItems: 'center', justifyContent: 'center' }}>
              <Ionicons name="briefcase" size={17} color={ACCENT} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontFamily: FontFamily.fredokaSemiBold, color: DARK, fontSize: 13 }}>Job Invitation</Text>
            ...
            {status !== 'pending' && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: (accepted ? GREEN : RED) + '18', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 999 }}>
                ...
                <Text style={{ fontSize: 10.5, fontFamily: FontFamily.fredokaSemiBold, color: accepted ? GREEN : RED }}>{accepted ? 'Accepted' : 'Declined'}</Text>
              </View>
            )}
          </View>
          <View style={{ paddingHorizontal: 12, paddingVertical: 11 }}>
            ...
            {showActions && (
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
                <TouchableOpacity disabled={inviteBusy} onPress={onDeclineInvite} activeOpacity={0.8}
                  ...
                  <Text style={{ color: MUTED, fontFamily: FontFamily.fredokaSemiBold, fontSize: 13 }}>Decline</Text>
                </TouchableOpacity>
                <TouchableOpacity disabled={inviteBusy} onPress={onAcceptInvite} activeOpacity={0.85}
                  style={{ flex: 1.5, paddingVertical: 10, borderRadius: 11, backgroundColor: ACCENT, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6 }}>
                  {inviteBusy ? <ActivityIndicator size="small" color="#fff" /> : (
                    <><Ionicons name="checkmark" size={15} color="#fff" /><Text style={{ color: '#fff', fontFamily: FontFamily.fredokaSemiBold, fontSize: 13 }}>Accept</Text></>
                  )}
                </TouchableOpacity>
              </View>
            )}
            ...
            {!isMine && accepted && (
              <TouchableOpacity onPress={onOpenInviteJob} activeOpacity={0.85}
                style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, marginTop: 12, paddingVertical: 10, borderRadius: 11, backgroundColor: ACCENT + '18' }}>
                <Ionicons name="arrow-forward-circle" size={16} color={ACCENT} />
                <Text style={{ color: ACCENT, fontFamily: FontFamily.fredokaSemiBold, fontSize: 13 }}>View job &amp; apply</Text>
              </TouchableOpacity>
            )}
