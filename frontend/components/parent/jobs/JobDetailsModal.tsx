...
function createPerkTagStyles(t: ThemeColor) {
  return StyleSheet.create({
    tag:  { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: t.surface, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20, borderWidth: 1 },
...
function createDetailsStyles(t: ThemeColor) {
  return StyleSheet.create({
    ...
    catPill: { backgroundColor: t.parentSoft, paddingHorizontal: 12, paddingVertical: 4, borderRadius: 8, borderWidth: 1, borderColor: t.parent + '33' },
    catText: { fontSize: 12, fontWeight: '700', color: t.parent },
    closeBtn:{ padding: 6, backgroundColor: t.surface, borderRadius: 16 },
    pendingBanner: { flexDirection: 'row', gap: 8, alignItems: 'center', marginHorizontal: 22, marginTop: 12, backgroundColor: t.warningSoft, padding: 12, borderRadius: 10 },
    pendingBannerText: { flex: 1, fontSize: 13, fontWeight: '600', color: t.warning },
    scroll:  { paddingHorizontal: 22, paddingTop: 8 },
    section:      { marginBottom: 24 },
    sectionHeader:{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12, paddingBottom: 8, borderBottomWidth: 1, borderBottomColor: t.line },
    sectionIconWrap: { width: 28, height: 28, borderRadius: 8, backgroundColor: t.parentSoft, alignItems: 'center', justifyContent: 'center' },
    sectionTitle: { fontSize: 14, fontWeight: '800', color: t.ink, textTransform: 'uppercase', letterSpacing: 0.6 },
    detailRow:    { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 },
    detailIconWrap: { width: 32, height: 32, borderRadius: 9, backgroundColor: t.parentSoft, alignItems: 'center', justifyContent: 'center' },
    detailLabel:  { fontSize: 10, fontWeight: '700', color: t.subtle, textTransform: 'uppercase', letterSpacing: 0.5 },
    detailValue:  { fontSize: 14, fontWeight: '600', color: t.ink },
    bodyText: { fontSize: 14, lineHeight: 22, color: t.muted },
    subLabel: { fontSize: 12, fontWeight: '700', color: t.inkMuted, marginBottom: 6 },
    prefRow:  { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
    prefChip: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: t.surface, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: t.line },
    prefChipText: { fontSize: 12, color: t.inkMuted, fontWeight: '600' },
    tagsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
    skillTag: { backgroundColor: t.parentSoft, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 8, borderWidth: 1, borderColor: t.parent + '22' },
    skillTagText: { fontSize: 12, fontWeight: '600', color: t.parent },
    infoBox:  { backgroundColor: t.surface, padding: 12, borderRadius: 10, borderWidth: 1, borderColor: t.line },
    reqRow:   { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: t.surface, padding: 10, borderRadius: 10, marginBottom: 8, borderWidth: 1, borderColor: t.line },
    reqIconBox:{ width: 34, height: 34, borderRadius: 9, backgroundColor: t.parentSoft, alignItems: 'center', justifyContent: 'center' },
    reqLabel: { fontSize: 10, fontWeight: '700', color: t.subtle, textTransform: 'uppercase' },
    reqValue: { fontSize: 13, fontWeight: '600', color: t.ink },
    verifiedReq: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: t.successSoft, padding: 10, borderRadius: 10, marginBottom: 8, borderWidth: 1, borderColor: t.success + '33' },
    verifiedReqText: { fontSize: 13, fontWeight: '700', color: t.success },
    salaryBox:  { backgroundColor: t.parent, padding: 20, borderRadius: 16, alignItems: 'center', marginBottom: 14 },
    ...
    contractTile: { flex: 1, backgroundColor: t.surface, padding: 12, borderRadius: 12, alignItems: 'center', borderWidth: 1, borderColor: t.line },
    contractLabel:{ fontSize: 10, fontWeight: '700', color: t.subtle, textTransform: 'uppercase', marginBottom: 4 },
    contractValue:{ fontSize: 14, fontWeight: '700', color: t.ink },
    meta:     { borderTopWidth: 1, borderTopColor: t.line, paddingTop: 16, paddingBottom: 8, gap: 4 },
    metaText: { fontSize: 12, color: t.subtle },
    footer:      { padding: 18, borderTopWidth: 1, borderTopColor: t.line },
    closeFullBtn:{ backgroundColor: t.parent, paddingVertical: 14, borderRadius: 12, alignItems: 'center' },
...
export function JobDetailsModal({ visible, onClose, job }: JobDetailsModalProps) {
  ...
  const s = useMemo(() => createDetailsStyles(t), [t]);
  const pk = useMemo(() => createPerkTagStyles(t), [t]);
  ...
  const skillList: string[] = (() => {
    ...
    if (typeof sn === 'string' && sn.trim()) return sn.split(',').map((x: string) => x.trim());
    return [];
  })();
  ...
  const Section = ({ icon, title, children }: { icon: React.ComponentProps<typeof Ionicons>['name']; title: string; children: React.ReactNode }) => (
    <View style={s.section}>
      <View style={s.sectionHeader}>
        <View style={s.sectionIconWrap}>
          <Ionicons name={icon} size={15} color={t.parent} />
        </View>
  ...
  const DetailRow = ({ icon, label, value }: { icon: React.ComponentProps<typeof Ionicons>['name']; label: string; value: string }) => (
  ...
  const PerkTag = ({ icon, label, color = '#D97706' as string }: { icon: React.ComponentProps<typeof Ionicons>['name']; label: string; color?: string }) => (
        ...
        <View style={s.card}>
          ...
          {isPending && (
            <View style={s.pendingBanner}>
              <Ionicons name="hourglass-outline" size={15} color={t.warning} />
          ...
          {isRejected && (
            <View style={[s.pendingBanner, { backgroundColor: t.dangerSoft }]}>
              <Ionicons name="close-circle-outline" size={15} color={t.danger} />
              <Text style={[s.pendingBannerText, { color: t.danger }]}>
          ...
          <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
            ...
            <Section icon="star-outline" title="Skills & Preferences">
              <View style={s.prefRow}>
                <View style={s.prefChip}>
                  <Ionicons name="language-outline" size={13} color={t.parent} />
                  <Text style={s.prefChipText}>Language: {displayLanguage}</Text>
                </View>
                {job.preferred_religion && (
                  <View style={s.prefChip}>
                    <Ionicons name="heart-outline" size={13} color={t.parent} />
            ...
            <Section icon="person-outline" title="Candidate Requirements">
              <View style={s.reqRow}>
                <View style={s.reqIconBox}>
                  <Ionicons name="person-outline" size={16} color={t.parent} />
                </View>
                <View>
              ...
              <View style={s.reqRow}>
                <View style={s.reqIconBox}>
                  <Ionicons name="star-outline" size={16} color={t.parent} />
                </View>
                <View>
              ...
              {isTrue(job.prefer_tesda_nc2) && (
                <View style={[s.verifiedReq, { backgroundColor: t.infoSoft }]}>
                  <Ionicons name="school" size={16} color={t.info} />
                  <Text style={[s.verifiedReqText, { color: t.info }]}>TESDA NC II Preferred</Text>
                </View>
              )}
            </Section>
            <Section icon="cash-outline" title="Compensation & Benefits">
              ...
              <View style={s.perksRow}>
                ...
                {isTrue(job.provides_sss)            && <PerkTag icon="checkmark-circle" label="SSS" color={t.success} />}
                {isTrue(job.provides_philhealth)     && <PerkTag icon="checkmark-circle" label="PhilHealth" color={t.success} />}
                {isTrue(job.provides_pagibig)        && <PerkTag icon="checkmark-circle" label="Pag-IBIG" color={t.success} />}
                {Number(job.vacation_days) > 0       && <PerkTag icon="airplane"        label={`${job.vacation_days} Vacation Days`} color={t.info} />}
                {Number(job.sick_days) > 0           && <PerkTag icon="medkit"          label={`${job.sick_days} Sick Leaves`} color={t.danger} />}
              </View>
              {job.benefits ? (
