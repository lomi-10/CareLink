...
export function JobPostModal({ 
  ...
  const handleGenerateDescription = () => {
    ...
    const generatedText = generateDescription(selectedCategory || null, selectedJobObjects);
  ...
  const renderDetailsStep = () => (
    <View style={styles.stepContent}>
      ...
      <ContractDetailsCard contractDuration={formData.contract_duration} onContractDurationChange={(val) => updateField('contract_duration', val)} disabled={isDisabled} />
    </View>
  );
                    ...
                    <TouchableOpacity 
                      ...
                      onPress={() => {
                        if (modalStep === 1) {
                          ...
                          if (!step1Result.isValid && step1Result.error) {
                            showStepError(step1Result.error);
                            return;
                          }
                        } else if (modalStep === 2) {
                          const step2Result = validateStep2();
                          if (!step2Result.isValid && step2Result.error) {
                            showStepError(step2Result.error);
                            return;
                          }
                        }
